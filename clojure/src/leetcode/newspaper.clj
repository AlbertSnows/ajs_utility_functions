...
(defn make-newspaper [phrases width]
	(let [trimmed-phrases (reduce #(conj %1 (trim %2)) [] paragraph)

				answer          "..."]
		answer))

(comment
	(let [width                      12                        ; convert to list for faster insertion time
				actual-width               (- width 2)
				trimmed-phrases            (reduce #(conj %1 (trim %2)) [] paragraph)
				...
				reorder-words
																	 (fn [{:keys [current-sum current-phrase reordered-words]} {:keys [word word-count]}]
																		 (let [new-current-sum (+ current-sum word-count)
																			 ...
																			 answer))
				reordered-words            (:reordered-words (reduce reorder-words {:current-sum     0
																																						:current-phrase  []
				...
				answer                     (reduce add-asterisks-and-spaces [] phrases-with-count)
				]
		answer)
	)